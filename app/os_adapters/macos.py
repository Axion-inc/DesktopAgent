...
class MacOSAdapter(OSAdapter):
    ...
    def take_screenshot(self, dest_path: str) -> None:
        ...
        try:
            subprocess.run(["screencapture", "-x", dest_path], check=True)
    ...
    def permissions_status(self) -> Dict[str, bool]:
        ...
        try:
            result = subprocess.run(["sqlite3", 
                                   ...
                                   "SELECT service FROM access WHERE client LIKE '%Terminal%' OR client LIKE '%Python%';"],
        ...
        try:
            ...
            result = subprocess.run(["screencapture", "-x", test_path], 
                                  capture_output=True, text=True, timeout=5)
            permissions["screen_recording"] = result.returncode == 0 and os.path.exists(test_path)
