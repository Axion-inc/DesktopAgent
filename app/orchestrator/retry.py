...
class RetryManager:
    ...
    def should_retry(self, run_data: Dict[str, Any], error: str) -> bool:
        ...
        if current_attempt >= max_attempts:
            logger.info(f"Run {run_data['id']} reached max attempts ({max_attempts})")
            return False
        ...
        if retry_config.get("only_idempotent", True):
            if not self._is_run_idempotent(run_data):
                logger.info(f"Run {run_data['id']} contains non-idempotent steps, skipping retry")
                return False
        
        # Check for non-retryable errors
        if self._is_permanent_error(error):
            logger.info(f"Run {run_data['id']} has permanent error, skipping retry: {error}")
            return False
    ...
    def calculate_delay(self, run_data: Dict[str, Any], attempt: int) -> int:
        ...
        max_delay = retry_config.get("max_backoff_ms", 300000)  # 5 minutes max
    ...
    def handle_failure(self, run_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        logger.info(f"Scheduling retry for run {run_id}, attempt {next_attempt} in {delay_ms}ms")
        
        # TODO: Schedule the retry (integrate with queue manager)
        return retry_data
    
    def record_attempt(self, run_id: str, attempt: int, success: bool, error: Optional[str] = None) -> None:
    ...
    def is_step_retryable(self, step: Dict[str, Any]) -> bool:
        ...
        if step_name in idempotent_steps:
            ...
            logger.warning(f"Unknown step type '{step_name}' - marking as non-retryable")
            return False
    ...
    def _is_run_idempotent(self, run_data: Dict[str, Any]) -> bool:
        ...
        return True  # Simplified - assume retry_config existence means it's safe
    ...
    def _update_metrics(self) -> None:
        ...
        runs_with_retries = sum(1 for attempts in runs_by_id.values() if len(attempts) > 1)
        
        self._metrics.update({
            "retry_rate_24h": runs_with_retries / total_runs if total_runs > 0 else 0.0,
