...
class RunQueue:
    ...
    def enqueue(self, run: QueuedRun) -> None:
        ...
        with self._lock:
            ...
            self._queue.put(run)
            logger.info(
                f"Enqueued run {run.id} in queue {self.config.name} with "
    ...
    def dequeue(self) -> Optional[QueuedRun]:
        ...
        with self._lock:
            ...
            logger.info(
                f"Dequeued run {run.id} from queue {self.config.name}")
            return run
    ...
    def complete(self, run_id: str) -> None:
        ...
        with self._lock:
            if run_id in self._running:
                ...
                logger.info(
                    f"Completed run {run_id} in queue {self.config.name}")
...
class QueueManager:
    ...
    def create_queue(self, name: str, max_concurrent: int = 2, max_queued: int = 50) -> RunQueue:
        ...
        logger.info(
            f"Created queue '{name}' with max_concurrent={max_concurrent}")
        return queue
    ...
    def set_tag_limit(self, tag: str, limit: int) -> None:
        ...
        logger.info(
            f"Set concurrency limit for tag '{tag}': {limit}")
    ...
    def enqueue(self, queue_name: str, run_data: Dict[str, Any]) -> str:
        ...
        # Generate run ID
        run_id = (
            f"run_{int(time.time() * 1000)}_"
            f"{run_data.get('template', 'unknown')}"
        )
            ...
            for queue_name, queue in self._queues.items():
                ...
                if run.concurrency_tag:
                    ...
                    if current_count >= tag_limit:
                        ...
                        queue._running.pop(run.id, None)
