...
class RBACManager:
    ...
    def _define_permissions(self) -> Dict[str, Permission]:
        ...
        permissions = {
            ...
            "read_runs": Permission(
                "read_runs", "View run history and details", Role.VIEWER),
            "read_metrics": Permission(
                "read_metrics", "View system metrics", Role.VIEWER),
            "read_dashboard": Permission(
                "read_dashboard", "Access dashboard", Role.VIEWER),

            # Create permissions (Runner+)
            "create_runs": Permission(
                "create_runs", "Create new runs", Role.RUNNER),
            "execute_templates": Permission(
                "execute_templates", "Execute plan templates", Role.RUNNER),
            ...
            "approve_runs": Permission(
                "approve_runs", "Approve dangerous operations", Role.EDITOR),
            "manage_templates": Permission(
                "manage_templates", "Create/modify templates", Role.EDITOR),
            "manage_schedules": Permission(
                "manage_schedules", "Manage cron schedules", Role.EDITOR),
            "respond_hitl": Permission(
                "respond_hitl", "Respond to HITL confirmations", Role.EDITOR),
            ...
            "manage_users": Permission(
                "manage_users", "Manage user accounts", Role.ADMIN),
            "manage_rbac": Permission(
                "manage_rbac", "Manage roles and permissions", Role.ADMIN),
            "manage_secrets": Permission(
                "manage_secrets", "Manage secret storage", Role.ADMIN),
            "manage_system": Permission(
                "manage_system", "System configuration", Role.ADMIN),
            "view_audit_log": Permission(
                "view_audit_log", "View audit log", Role.ADMIN),

            # Queue management (Admin only)
            "manage_queues": Permission(
                "manage_queues", "Manage run queues", Role.ADMIN),
            "pause_queues": Permission(
                "pause_queues", "Pause/resume queues", Role.ADMIN),
        }
        return permissions
    ...
    def get_session_user(self, token: str) -> Optional[User]:
        ...
        cur.execute("""
            SELECT s.user_id, s.role, s.expires_at, u.username, u.created_at,
                   u.enabled
    ...
    def require_permission(self, user: Optional[User], permission_name: str,
                           resource: str = "") -> None:
    ...
    def _log_audit_event(self, action: str, user_id: Optional[str],
    ...
    def _log_denial(self, user_id: Optional[str], permission: str,
                    resource: str):
