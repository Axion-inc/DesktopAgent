...
def compute_metrics() -> Dict[str, float]:
    ...
    cur.execute("""
        SELECT COUNT(*) FROM run_steps
        WHERE name IN ('wait_for_element', 'assert_element', 'assert_text', 'assert_file_exists', 'assert_pdf_pages')
        AND finished_at >= datetime('now','-1 day')
    """)
    ...
    cur.execute("""
        SELECT COUNT(*) FROM run_steps
        WHERE name IN ('wait_for_element', 'assert_element', 'assert_text', 'assert_file_exists', 'assert_pdf_pages')
        AND (output_json LIKE '%"passed":true%' OR output_json LIKE '%"status":"PASS"%' OR output_json LIKE '%"status":"RETRY"%')
        AND finished_at >= datetime('now','-1 day')
    """)
    ...
    # This is a placeholder implementation - would need capability negotiation logging
    cur.execute("""
        SELECT COUNT(*) FROM run_steps
    ...
    out = {
        ...
        # Phase 3 metrics
        "verifier_pass_rate_24h": round((verifier_steps_passed) / (verifier_steps_total or 1), 2),
        "schema_captures_24h": schema_captures_24h,
        "web_upload_success_rate_24h": round((web_upload_success) / (web_upload_total or 1), 2),
